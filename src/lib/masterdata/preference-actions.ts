...
async function requireEntity(
  ...
  const entity =
    kind === "instructor"
      ? await tx.instructor.findFirst({ where })
      : kind === "student"
        ? await tx.student.findFirst({ where })
        : kind === "course"
          ? await tx.course.findFirst({ where })
          : kind === "room"
            ? await tx.room.findFirst({ where })
            : kind === "feature"
              ? await tx.roomFeature.findFirst({ where })
              : kind === "cohort"
                ? await tx.studentCohort.findFirst({ where })
                : kind === "group"
                  ? await tx.teachingGroup.findFirst({ where })
                  : null;
...
export async function saveCourseCompetence(data: FormData) {
  ...
  await prisma.$transaction(async (tx) => {
    await requireEntity(tx, tid, "instructor", instructorId);
    await requireEntity(tx, tid, "course", courseId);
    const where = {
      tenantId_instructorId_courseId: { tenantId: tid, instructorId, courseId },
    };
...
export async function saveTimePreference(data: FormData) {
  ...
  const startMinute = minute(data, "startMinute"),
    endMinute = minute(data, "endMinute");
...
function roomFeatureCodeBase(
  ...
  const stem = normalized.slice(0, 4) || "ITEM";
  const prefix = type === "EQUIPMENT" ? "EQ" : "FT";

  return `${prefix}-${stem}`;
}

async function generateRoomFeatureCode(
  tx: Prisma.TransactionClient,
  tenantId: string,
  type: "EQUIPMENT" | "FEATURE",
  name: string,
) {
  const base = roomFeatureCodeBase(type, name);
  let candidate = base;
  let suffix = 2;

  while (
    await tx.roomFeature.findUnique({
      where: {
        tenantId_code: {
          tenantId,
          code: candidate,
...
export async function saveRoomFeature(data: FormData) {
  ...
  if (type !== "EQUIPMENT" && type !== "FEATURE") {
    throw new Error("Invalid room feature type.");
  }

  await prisma.$transaction(async (tx) => {
    const existing = await tx.roomFeature.findFirst({
      where: {
        tenantId: tid,
        type,
        name: {
    ...
    const code = await generateRoomFeatureCode(tx, tid, type, name);

    const after = await tx.roomFeature.create({
      data: {
        tenantId: tid,
        code,
        name,
...
export async function saveRoomInventory(data: FormData) {
  ...
  const featureId = text(data, "featureId");
  const quantity = integer(data, "quantity", 0);

  await prisma.$transaction(async (tx) => {
    await requireEntity(tx, tid, "room", roomId);
    await requireEntity(tx, tid, "feature", featureId);
    ...
    if (quantity <= 0) {
      if (before) {
        await tx.roomFeatureValue.delete({ where });

        await writeAuditEvent(tx, {
          tenantId: tid,
          eventType: "DEACTIVATED",
          entityType: "RoomFeatureValue",
          entityId: before.id,
          source: "masterdata.preferences",
          description: "RoomFeatureValue removed",
          beforeState: before,
          afterState: null,
        });
      }
    ...
    const after = await tx.roomFeatureValue.upsert({
      where,
      create: {
        tenantId: tid,
        roomId,
...
export async function saveRoomRequirement(data: FormData) {
  const tid = await tenantId(),
    [kind, id] = text(data, "target").split(":"),
    featureId = text(data, "featureId");
  if (!["course", "student", "instructor"].includes(kind))
    throw new Error("Invalid requirement target");
  const quantity = integer(data, "quantity"),
    weight = integer(data, "weight");
  await prisma.$transaction(async (tx) => {
    await requireEntity(tx, tid, kind, id);
    await requireEntity(tx, tid, "feature", featureId);
    const after = await tx.roomRequirement.create({
      data: {
        tenantId: tid,
        featureId,
        quantity,
...
export async function togglePreference(data: FormData) {
  ...
  await prisma.$transaction(async (tx) => {
    if (kind === "time") {
      ...
      const after = await tx.planningRule.update({
        where: { id },
        data: { active: !before.active },
      });
      await audit(tx, tid, "PlanningRule", after, before);
    } else if (kind === "room") {
      const before = await tx.roomRequirement.findFirstOrThrow({
        where: { tenantId: tid, id },
      });
      const after = await tx.roomRequirement.update({
        where: { id },
        data: { active: !before.active },
      });
      await audit(tx, tid, "RoomRequirement", after, before);
    } else if (kind === "competence") {
      const before = await tx.instructorCourse.findFirstOrThrow({
        where: { tenantId: tid, id },
      });
      const after = await tx.instructorCourse.update({
        where: { id },
        data: { active: !before.active },
      });
      await audit(tx, tid, "InstructorCourse", after, before);
