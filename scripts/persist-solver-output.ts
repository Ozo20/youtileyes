...
async function main() {
 ...
 if (!scenario) {
   throw new Error(
     `PlanScenario ${output.plan_scenario_id} was not found`,
   );
 }

 if (scenario.tenantId !== output.tenant_id) {
   throw new Error(
     "Tenant mismatch between solver output and PlanScenario",
   );
 }
