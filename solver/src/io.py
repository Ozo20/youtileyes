...
def solver_input_from_dict(data: dict[str, Any]) -> SolverInput:
   ...
   teaching_groups = tuple(
       TeachingGroupInput(
           ...
           student_ids=tuple(
               str(value) for value in item["studentIds"]
           ),
           duration_minutes=int(item["durationMinutes"]),
           allowed_room_ids=tuple(
               str(value) for value in item["allowedRoomIds"]
           ),
       )
   ...
   student_load_profile = LoadProfileInput(
       max_teaching_minutes_per_day=profile.get(
           "maxTeachingMinutesPerDay"
       ),
       max_continuous_teaching_minutes=profile.get(
           "maxContinuousTeachingMinutes"
       ),
       min_break_minutes=int(profile.get("minBreakMinutes", 0)),
       min_break_after_double_minutes=int(
           profile.get("minBreakAfterDoubleMinutes", 0)
       ),
       ...
       travel_consumes_break_time=bool(
           profile.get("travelConsumesBreakTime", True)
       ),
   )
   ...
   return SolverInput(
       ...
       start_times=tuple(
           int(value) for value in _required(data, "startTimes")
       ),
